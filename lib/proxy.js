    ...
    configs.forEach((config) => {
        if (!proxyHit) {
            ...
            if (proxyHit) {
                ...
                console.log(
                    `Proxying request ${req.method}, ${req.url} to ${config.target}`
                );

                // config.target = `${protocol}://${config.target}`;

                const configToSend = {...config}

                delete configToSend["forward"];
                delete configToSend["protocol"];
                ...
                if (proxyServer === null) {
                    console.log(config);
                    // proxyServer = httpProxy.createProxyServer(config);
                    proxyServer = httpProxy.createProxyServer();
                }

                proxyServer.web(req, res, configToSend, (e, req, res) => {
